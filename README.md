...
- **系统托盘**：任务栏右下角常驻小图标——单击切换隐藏/召回；右键菜单可选桌宠所在显示器（多屏支持）、开机启动、创建快捷方式、退出。
- **一键隐藏**：桌宠缩成一颗可拖动的小悬浮球（记住位置）。单击/双击/右键悬浮球、点托盘图标或按 `Ctrl+Alt+H` 都能召回。
- **多显示器**：托盘菜单「桌宠所在显示器」里选屏，桌宠和悬浮球一起搬家，选择会记住。
- **开机启动**：点 🚀 或托盘菜单切换（亮起=已开启），写入当前用户注册表 Run 项，不需要管理员权限。
